...
export class QuickSettingsLayoutEditor {
    ...
    open(tiles) {
        ...
        content.add_child(new St.Label({
            text: 'Tap to select · Drag a control to move it',
    ...
    _visibleTiles() { return this._tiles.filter(tile => !tile.hidden); }
    _spanFor(tile) { return tile.slider ? 4 : tile.mode === 'wide' ? 2 : 1; }
    _selectedTile() { return this._tiles.find(tile => tile.key === this._selectedKey) ?? null; }
    _actionButton(label, styleClass, callback, reactive = true) {
    ...
    _rebuild() {
        if (!this._gridWidget || !this._trayInner)
            return;
        this._gridWidget.destroy_all_children();
        this._trayInner.destroy_all_children();
        this._actors.clear();
        const layout = this._gridWidget.layout_manager;
        let row = 0;
        let column = 0;
        ...
        if (tray)
            tray.visible = hiddenTiles.length > 0;
    ...
    _makeTile(tile, hidden) {
        ...
        if (tile.key === this._selectedKey)
            style += ' liquid-glass-editor-tile-selected';
        ...
        if (isCompact)
            button.set_size(68, 68);
        else if (hidden)
            button.set_height(60);
    ...
    _bindDrag(target, actor, tile, hidden) {
        ...
        target.connect('motion-event', () => {
            if (!this._drag || this._drag.actor !== actor)
                return Clutter.EVENT_PROPAGATE;
            const [x, y] = global.get_pointer();
            if (!this._drag.active) {
                if (Math.abs(x - this._drag.grabX) < DRAG_THRESHOLD && Math.abs(y - this._drag.grabY) < DRAG_THRESHOLD)
                    return Clutter.EVENT_STOP;
                this._beginDrag(this._drag);
            }
            this._moveGhost(x, y);
            this._updateGap();
            return Clutter.EVENT_STOP;
        ...
        target.connect('button-release-event', () => {
            ...
            this._endDrag();
            if (active && drop)
                this._finishDrop(tile, drop[0], drop[1], origin);
            else {
                this._selectedKey = tile.key;
                this._rebuild();
            }
            return Clutter.EVENT_STOP;
        });
        if (hidden)
            target.connect('clicked', () => this._setHidden(tile, false));
    }
    _beginDrag(drag) {
        drag.active = true;
    ...
    _setMode(tile, mode) { if (tile.slider && mode === 'circle')
        return; tile.mode = mode; this._rebuild(); }
    _setHidden(tile, hidden) { tile.hidden = hidden; this._selectedKey = tile.key; this._rebuild(); }
    _updateGap() {
        ...
        for (let index = 0; index < others.length; index++) {
            ...
            if (!best || distance < best.distance)
                best = { index, key: others[index].key, rect, distance };
        }
        if (best) {
        ...
        if (this._drag.targetKey !== best?.key) {
            if (this._drag.targetKey)
                this._actors.get(this._drag.targetKey)?.remove_style_class_name('liquid-glass-editor-drop-target');
            if (best?.key)
                this._actors.get(best.key)?.add_style_class_name('liquid-glass-editor-drop-target');
            this._drag.targetKey = best?.key ?? null;
        }
        if (gap === this._drag.gapIndex)
            return;
    ...
    _startGhost(source) {
        ...
        try {
            global.stage.set_child_above_sibling(ghost, null);
            source.opacity = 92;
            source.add_style_class_name('liquid-glass-editor-tile-dragging');
        }
        catch (e) { }
        this._ghost = ghost;
    }
    _moveGhost(x, y) { if (!this._ghost)
        return; const [w, h] = this._ghost.get_size(); this._ghost.set_position(Math.round(x - w / 2), Math.round(y - h / 2)); }
    _ghostCenter() { if (!this._ghost)
        return null; const [x, y] = this._ghost.get_transformed_position(); const [w, h] = this._ghost.get_transformed_size(); return [x + w / 2, y + h / 2]; }
    _endDrag() {
        if (this._drag) {
            try {
    ...
    catch (e) { } this._ghost = null; }
    _rect(actor) { const [x, y] = actor.get_transformed_position(); const [w, h] = actor.get_transformed_size(); return { x, y, w, h }; }
    _contains(rect, x, y) { return rect.w > 0 && rect.h > 0 && x >= rect.x && x <= rect.x + rect.w && y >= rect.y && y <= rect.y + rect.h; }
    _save() {
        ...
        try {
            this._settings.set_string('quick-settings-layout', JSON.stringify({ version: 3, items, hidden }));
        }
        catch (e) { }
        this.close();
        this._onSaved();
