...
interface OsdState {
  osdWindow: any; // GNOME内部のOsdWindowクラス（厳密な型定義がないためanyで許容）
  targetBox: St.Widget;
  bgActor: St.Widget | null;
  clipBox: St.Widget;
  blurEffect: Shell.BlurEffect | null;
  effect: LiquidEffect | null;
  bgClone: Clutter.Clone | null;
  windowClonesContainer: Clutter.Actor | null;
  overviewCloneContainer: Clutter.Actor | null;
  _windowClones: Map<Clutter.Actor, Clutter.Clone>;
  _overviewClone: Clutter.Clone | null;
  _appDisplayClone: Clutter.Clone | null;
  _searchClone: Clutter.Clone | null;
  _stableBaseW?: number;
  _stableBaseH?: number;
  _lastBgW?: number;
  _lastBgH?: number;
  ...
  _destroyId: number;
  _visibilitySignalIds: { target: any, id: number }[];
}
...
export class OsdManager {
  ...
  _setupOsdEffect(osdWindow: any) {
    if (!(osdWindow as any)._liquidOrigEase) {
      let actorToPatch = osdWindow.actor || osdWindow;
      if (typeof actorToPatch.ease === 'function') {
        (actorToPatch as any)._liquidOrigEase = actorToPatch.ease;
        actorToPatch.ease = function(props: any) {
          if (props.scale_x !== undefined) delete props.scale_x;
          if (props.scale_y !== undefined) delete props.scale_y;
    ...
    let state: OsdState = {
      ...
      _windowClones: new Map<Clutter.Actor, Clutter.Clone>(),
      _overviewClone: null,
    ...
    const connectVisibilitySignal = (target: any, signalName: string) => {
  ...
  _isOsdStateVisible(state: OsdState) {
  ...
  _syncOsdVisibilityAndOpacity(state: OsdState) {
    if (!state.bgActor || !state.targetBox || !state.osdWindow) return;
  ...
  _syncActorProperties(source: Clutter.Actor, clone: Clutter.Clone) {
    ...
    if (state.bgClone && state.windowClonesContainer) {
      ...
      if (!isOverview) {
        ...
        for (let w of windows) {
          ...
          if (clone) state.windowClonesContainer.set_child_at_index(clone, zIndex);
          zIndex++;
        }
      } else {
        state.bgClone.show();
  ...
  _startAdaptiveColorSampling() {
    if (!this._adaptiveConfig.enabled || !this._hasVisibleOsd()) return;
