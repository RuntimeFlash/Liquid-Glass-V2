...
export class DashManager {
  ...
  private _signalActors: Map<number, Clutter.Actor>;
  private _overviewSignalIds: number[];
  ...
  private fboContainer: Clutter.Actor | null = null;
  ...
  _applyEffect() {
    ...
    let getAncestorByName = (actor: Clutter.Actor, name: string): Clutter.Actor | null => {
      let parent = actor.get_parent();
      while (parent) {
        if (parent.get_name && parent.get_name() === name) return parent;
    ...
    let childSignals = new Map<Clutter.Actor, number[]>();

    let connectActorSignals = (actor: Clutter.Actor) => {
      if (!actor) return;
      let ids: number[] = [];
      ...
      try {
        let sig1 = actor.connect('notify::slide-x', () => { if (this._isEffectActive) startFrameSync(); });
        let sig2 = actor.connect('notify::slide-y', () => { if (this._isEffectActive) startFrameSync(); });
        ids.push(sig1, sig2);
    ...
    let disconnectActorSignals = (actor: Clutter.Actor) => {
    ...
    let connectChildrenRecursively = (parentActor: Clutter.Actor) => {
    ...
    this._overviewSignalIds.push(Main.overview.connect('hidden', () => {
      if (this._isEffectActive) startFrameSync();
    }));
  ...
  _syncGeometry() {
    ...
    let findDashBackground = (actor: St.Widget | Clutter.Actor): St.Widget | null => {
      if (!actor) return null;
      // Convert to any to safely check has_style_class_name
      let stActor = actor as any;
      if (
        ...
        return stActor as St.Widget;
      }
      if (!actor.get_children) return null;
      let children = actor.get_children();
      for (let i = 0; i < children.length; i++) {
        let found = findDashBackground(children[i] as St.Widget | Clutter.Actor);
        if (found) return found;
      }
      return null;
    };

    let isIgnoredDockActor = (actor: St.Widget | Clutter.Actor): boolean => {
      let stActor = actor as any;
      return !!(
        actor === this.bgActor ||
        actor === this.clipBox ||
        actor === this.fboContainer ||
        (stActor.has_style_class_name && (
    ...
    let unionBounds = (a: { x: number, y: number, w: number, h: number } | null,
    ...
    let getVisibleContentBounds = (actor: St.Widget | Clutter.Actor): { x: number, y: number, w: number, h: number } | null => {
      if (!actor || isIgnoredDockActor(actor)) return null;

      let childBounds: { x: number, y: number, w: number, h: number } | null = null;
      if (actor.get_children) {
        for (let child of actor.get_children()) {
          childBounds = unionBounds(childBounds, getVisibleContentBounds(child as St.Widget | Clutter.Actor));
        }
      }

      if (childBounds) return childBounds;

      if (!actor.visible || !actor.mapped || actor.opacity === 0) return null;

      let [w, h] = actor.get_size();
      let [x, y] = actor.get_transformed_position();
      if (Number.isNaN(x) || Number.isNaN(y) || Number.isNaN(w) || Number.isNaN(h) || w < 4 || h < 4) return null;

      return { x, y, w, h };
    };

    let sourceActor: St.Widget = this.targetActor;
  ...
  _removeEffect() {
    ...
    try {
      for (let sigId of this._signals) {
        try { this._signalActors.get(sigId)?.disconnect(sigId); } catch (e) {}
      }
    ...
    for (let signalId of this._overviewSignalIds) {
      try { Main.overview.disconnect(signalId); } catch (e) {}
    }
    this._overviewSignalIds = [];
  ...
  private _findReferenceActor(actor: Clutter.Actor): Clutter.Actor | null {
    ...
    if (
      str.includes('IndicatorDrawingArea') ||
      name === 'd2daBackground' ||
      name === 'DockBackground' ||
      (stActor.has_style_class_name && (
        stActor.has_style_class_name('dash-background') ||
    ...
    let getInnermostIconActor = (root: Clutter.Actor): Clutter.Actor => {
      let current = root;
      while (current && typeof current.get_children === 'function') {
        let children = current.get_children();
        let bestChild: Clutter.Actor | null = null;
        for (let child of children) {
          let [cw, ch] = child.get_size();
          let cStr = child.toString();
          let cActor = child as any;
          if (
            cStr.includes('IndicatorDrawingArea') ||
            (cActor.has_style_class_name && (
              cActor.has_style_class_name('number-overlay') ||
    ...
    if (
      stActor.has_style_class_name && (
        stActor.has_style_class_name('overview-icon') ||
