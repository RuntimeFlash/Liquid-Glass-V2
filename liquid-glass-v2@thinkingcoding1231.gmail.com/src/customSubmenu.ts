...
type MenuKind = 'bluetooth' | 'wifi' | 'caffeine' | 'power' | 'system';
...
export class CustomSubmenuRenderer {
  manager: any;
  _floatingWindow: any;
  _pendingActions: Map<string, string>;
  _refreshTimerIds: number[];
  _wifiScanActive: boolean;
  _wifiScanGeneration: number;
  ...
  _getKindFromCard(stCard: any): MenuKind | null {
    let text = `${stCard?.title || ''} ${stCard?.label || ''}`.toLowerCase();
    if (text.includes('bluetooth')) return 'bluetooth';
    if (text.includes('wi-fi') || text.includes('wifi') || text.includes('wireless')) return 'wifi';
    if (text.includes('caffeine')) return 'caffeine';
    if (text.includes('power')) return 'power';
    return null;
  }

  _getSourceBounds(sourceActor: any) {
    try {
  ...
  _showPanel(
    rowBuilder: () => ControlRow[],
    settingsPanel: MenuKind,
    sourceCard?: any
  ) {
    ...
    let serializeRows = (rows: ControlRow[]) => {
    ...
    let refreshRows = () => {
      ...
      if (newHash === lastRowsHash) return;
    ...
    try {
      allocationSyncId = rowsBox.connect('notify::allocation', () => {
        if (allocationSyncId) {
          try { rowsBox.disconnect(allocationSyncId); } catch (e) {}
          allocationSyncId = 0;
        }
        shell.syncHeight(rowsBox, scrollView);
      });
    ...
    let pollId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 300, () => {
      try {
        if (settingsPanel === 'wifi') this._syncNativeWifiItems(sourceCard);
        refreshRows();
      } catch (e) {}
  ...
  _createFloatingShell(sourceCard?: any) {
    ...
    let sourceActor = (Main.panel.statusArea.quickSettings as any)?.container || Main.panel.statusArea.quickSettings;
    ...
    let cloneMap = new Map<Clutter.Actor, Clutter.Clone>();
    let zIndex = 0;
    ...
    for (let windowActor of global.get_window_actors()) {
      ...
      if (wRight < cullLeft || windowActor.x > cullRight || wBottom < cullTop || windowActor.y > cullBottom) continue;
      ...
      try { windowClonesContainer.set_child_at_index(clone, zIndex); } catch (e) {}
      zIndex++;
    }
    ...
    let effect = new LiquidEffect({ extensionPath: this.manager.extensionPath, settings: this.manager._settings } as any);
    let tintColor = this.manager._hexToColorArray(this.manager._settings.get_string('quick-settings-tint-color')) as [number, number, number];
    ...
    let syncHeight = (contentActor: any, scrollActor?: any) => {
      try {
    ...
    let closeFloatingWindow = (reopenQuickSettings = true, immediate = false) => {
      ...
      if (!reopenQuickSettings) {
        try { this.manager.finishQuickSettingsSubmenuWithoutReopen?.(); } catch (e) {}
      }
      ...
      let finishCleanup = () => {
        if (cleanupDone) return GLib.SOURCE_REMOVE;
        cleanupDone = true;

        try { scrim.destroy(); } catch (e) {}
        try { winActor.destroy(); } catch (e) {}
        try { glassActor.destroy(); } catch (e) {}
        for (let clone of cloneMap.values()) {
          try { clone.destroy(); } catch (e) {}
        }
        cloneMap.clear();
        try { effect.cleanup(); } catch (e) {}
        this._floatingWindow = null;
    ...
    winActor.connect('key-press-event', (_actor: any, event: any) => {
    ...
    return {
      ...
      present: () => {
        ...
        try { this.manager.closeQuickSettingsForSubmenu?.(true); } catch (e) {
          try { this.manager.menu.close(false); } catch (e2) {}
        }
  ...
  _buildBluetoothRows(): ControlRow[] {
    let indicator = (Main.panel.statusArea.quickSettings as any)?._bluetooth;
    ...
    let rows: ControlRow[] = [{
      ...
      onActivate: () => {
        ...
        try { client.toggleActive(); } catch (e) {}
      },
    }];

    if (!client.available) return rows;
  ...
  _buildWifiRows(stCard: any): ControlRow[] {
    let rows: ControlRow[] = [];
    ...
    let parsedRows = menuItems.map((item) => this._rowFromNativeItem(item, 'wifi')).filter((row): row is ControlRow => {
      if (!row) return false;
  ...
  _buildGenericRows(stCard: any, kind: MenuKind): ControlRow[] {
    let rows: ControlRow[] = [];
    ...
    if (kind !== 'power' && kind !== 'system') {
      rows.push({
        ...
        onActivate: () => {
          try {
            if (typeof stCard?.activate === 'function') stCard.activate(Clutter.get_current_event?.());
            else if (typeof stCard?._toggle?.activate === 'function') stCard._toggle.activate(Clutter.get_current_event?.());
            else if (typeof stCard?.emit === 'function') stCard.emit('activate', Clutter.get_current_event?.());
          } catch (e) {}
        }
      });
    }
            ...
            row.onActivate = () => {
                try {
                    if (kind === 'system' || kind === 'power') {
                        ...
                        GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
                            try {
                                if (typeof item.activate === 'function') item.activate(event);
                                else if (typeof item.emit === 'function') item.emit('activate', event);
                            } catch (e) {}
                            return GLib.SOURCE_REMOVE;
                        });
                        return;
                    } else {
                        if (typeof item.activate === 'function') item.activate(Clutter.get_current_event?.());
                        else if (typeof item.emit === 'function') item.emit('activate', Clutter.get_current_event?.());
                    }
                } catch (e) {}
            };
        }
        return row;
    }).filter((row): row is ControlRow => {
      if (!row) return false;
      let key = row.title.toLowerCase();
      if (seen.has(key)) return false;
      seen.add(key);
  ...
  _createControlRow(row: ControlRow, refreshRows: () => void) {
    let styleClass = 'liquid-glass-control-row';
    if (row.active) styleClass += ' liquid-glass-control-row-active';
    if (row.pending) styleClass += ' liquid-glass-control-row-pending';
    if (row.switchRow) styleClass += ' liquid-glass-control-row-switch';
    ...
    if (row.active) iconStyleClass += ' liquid-glass-control-icon-active';
    if (row.pending) iconStyleClass += ' liquid-glass-control-icon-pending';
    ...
    if (row.subtitle) {
      ...
      if (row.pending) subtitleClass += ' liquid-glass-control-subtitle-pending';
  ...
  _rowFromNativeItem(item: any, kind: string = 'wifi'): ControlRow | null {
    if (!item || item.visible === false || item.reactive === false) return null;

    let labels = this._extractLabels(item);
    let title = labels[0];
    if (!title || this._isNativeUtilityLabel(title)) return null;

    let subtitle = labels.slice(1).find((label) => label !== title && !this._isNativeUtilityLabel(label));
    if (title.toLowerCase().includes('all networks')) return null;
    ...
    if (kind === 'caffeine') fallbackIcon = 'caffeine-cup-full-symbolic';
    else if (kind === 'power') fallbackIcon = 'power-profile-balanced-symbolic';
    else if (kind === 'bluetooth') fallbackIcon = 'bluetooth-active-symbolic';
    else if (kind === 'system') fallbackIcon = 'system-shutdown-symbolic';
    else if (kind === 'wifi') fallbackIcon = active ? 'network-wireless-signal-excellent-symbolic' : 'network-wireless-signal-good-symbolic';
  ...
  _getNativeMenuItems(stCard: any): any[] {
  ...
  _createSectionLabel(title: string, scanning = false) {
    let box = new St.BoxLayout({
  ...
  _startWifiPolling(stCard: any, refreshRows: () => void) {
  ...
  async _triggerWifiScan(stCard: any) {
    try {
      ...
      for (let device of devices) {
        try { device.request_scan_async(null, null); } catch (e) {}
      }
  ...
  _syncNativeWifiItems(stCard: any) {
    try {
      let wifiToggle = this._getWifiToggle(stCard);
  ...
  _getWifiToggle(stCard: any) {
    if (stCard && (typeof stCard._scanDevices === 'function' || stCard._items))
      return stCard;
    return (Main.panel.statusArea.quickSettings as any)?._network?._wirelessToggle || stCard;
  }

  _getWifiDeviceItems(wifiToggle: any): any[] {
    let items: any[] = [];
    try {
  ...
  _markPending(key: string, label: string) {
  ...
  _findWifiPowerItem(items: any[], currentlyActive: boolean) {
  ...
  _extractIconName(actor: any): string {
    let iconName = '';
    let visit = (node: any) => {
  ...
  _isActorChecked(actor: any) {
    try { if (actor.checked === true) return true; } catch (e) {}
    try { if (actor.get_checked?.() === true) return true; } catch (e) {}
    try { if (actor.has_style_pseudo_class?.('checked')) return true; } catch (e) {}
    try { if (actor.has_style_class_name?.('checked')) return true; } catch (e) {}
    return false;
  }

  _isNativeUtilityLabel(label: string) {
