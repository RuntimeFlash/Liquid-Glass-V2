...
export class UIManager {
  ...
  private _menuOpenScale: number = 1.08;

  private _disabledClipActors: Map<Clutter.Actor, boolean> = new Map();

  constructor(extensionPath: string, settings: Gio.Settings, panelItem: any = Main.panel.statusArea.dateMenu) {
    ...
    this.targetActor = panelItem.menu.actor as St.Widget;
    this.menu = panelItem.menu;

    // Target for animations and visual offsets (The inner content)
    this.animActor = panelItem.menu.box as St.Widget;
  ...
  _destroyWindowClones() {
    ...
    if (this._overviewClone) { try { this._overviewClone.destroy(); } catch (e) {} this._overviewClone = null; }
    if (this._appDisplayClone) { try { this._appDisplayClone.destroy(); } catch (e) {} this._appDisplayClone = null; }
    if (this._searchClone) { try { this._searchClone.destroy(); } catch (e) {} this._searchClone = null; }
  }
  ...
  _syncGeometry() {
    ...
    if (this.bgClone && this.windowClonesContainer && !Number.isNaN(bgX) && !Number.isNaN(bgY)) {
      ...
      if (!isOverview) {
        ...
        let cloneIndexMap: Map<Clutter.Actor, number> | null = null;
        let getCloneIndex = (c: Clutter.Actor): number => {
          if (!cloneIndexMap) {
        ...
        for (let w of windows) {
          try {
            ...
            if (clone) clone.set_position(w.x, w.y);
    ...
    if (this._tickId === 0) {
      ...
      let runTick = () => {
        ...
        if ((this as any)._stampFrame <= 3 || stopped) {
          log(`[LG-STAMP] tick #${(this as any)._stampFrame} isClosing=${isClosing} s=${s.toFixed(4)} p=${p.toFixed(4)} scale=${currentScale.toFixed(4)} opacity=${opacity.toFixed(1)} stopped=${stopped}`);
        }
