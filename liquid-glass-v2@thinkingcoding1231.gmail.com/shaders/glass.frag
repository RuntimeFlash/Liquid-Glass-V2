...
float getHeightFromD(float d, float r, float zScale) {
...
// Computes the height gradient (slope) analytically.
vec2 heightGradient(vec2 p, vec2 b, float r, float zScale, vec2 resolution, float d) {
    float e = gradientStep(resolution);
    float dH_dd = (getHeightFromD(d + e, r, zScale) - getHeightFromD(d - e, r, zScale)) / (2.0 * e);
    return dH_dd * gradSDF(p, b, r);
}
...
vec2 getDisplacement(float d, vec3 normal, vec2 resolution) {
    ...
    float max_disp = 0.30;
    if (dot(displacement, displacement) > max_disp * max_disp) {
...
void main() {
    ...
    // EARLY EXIT: Skip all expensive computation (height gradients, refraction,
    // chromatic aberration with 12 texture lookups, lighting) for pixels that are
    // completely outside the glass region. The margin covers anti-aliasing
    // feathering + rim lighting so those visual effects are preserved.
    float earlyExitMargin = edgeFeather + rim_width + 2.0;
    if (d > earlyExitMargin) {
        discard;
    }
    ...
    if (d >= -corner_radius) {
        gradH = heightGradient(local_pos, box_size, corner_radius, max_z, resolution, d);
        normal = getNormal(gradH);
        disp = getDisplacement(d, normal, resolution) * edgeDampen;
    }
    ...
    vec2 chromaDir = dispSq > 1e-10 ? normalize(disp) : vec2(0.0);
    ...
    vec3 refractedRgb;
    if (dispSq < 1e-10) {
        // Fast path: 1 texture lookup instead of 12
        vec2 safeUv = SAFE(uv);
        vec4 sample = texture2D(cogl_sampler, safeUv);
        refractedRgb = sample.rgb;
    } else {
        bool needsAA = (d > -edgeFeather * 4.0);
        if (needsAA) {
        ...
        } else if (chroma_strength < 0.00001) {
            // Chromatic aberration disabled: uvR == uvB == refractedUv, so a
            // single tap is identical to the 3-tap per-channel sampling below.
            refractedRgb = texture2D(cogl_sampler, SAFE(refractedUv)).rgb;
        } else {
            // Fast path: 3 lookups instead of 12
            refractedRgb = vec3(
                texture2D(cogl_sampler, SAFE(uvR)).r,
                texture2D(cogl_sampler, SAFE(uvG)).g,
                texture2D(cogl_sampler, SAFE(uvB)).b
            );
        }
    }
    ...
    if (specular_intensity > 0.00001) {
        float specularDot = max(dot(reflectDir, viewDir), 0.0);
        specularLight = pow(specularDot, max(shininess, 1.0));
    ...
    if (sheen_intensity > 0.00001) {
        ...
        sheenColor = vec3(1.0) * surfaceSheen * sheen_intensity;
    }
